...
class CreditServiceTest {
  ...
  @Test
  fun `should not create credit when the number of installments is greater than 48`() {
    //given
    val invalidNumberOfInstallments = 49
    val credit: Credit = buildCredit(numberOfInstallments = invalidNumberOfInstallments)

    every { creditRepository.save(credit) } answers { credit }
    //when
    Assertions.assertThatThrownBy { creditService.save(credit) }
            .isInstanceOf(BusinessException::class.java)
            .hasMessage("Number of installments must be less than or equal to 48.")
    //then
    verify(exactly = 0) { creditRepository.save(any()) }
  }
